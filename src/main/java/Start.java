...
import geography.MSAtoWBANMapper;
...
public class Start {
	...
	public static void main() {
		...
		HashMap<String, ArrayList<Integer>> msaToFIPSMap = census.Parser.getMSAtoFIPSMap(MSA_DELINEATION_DATA);
		
		System.out.println("Connecting MSAs and WBANs");
		HashMap<MetropolitanStatisticalArea, ArrayList<WBANStation>> msaToWBANMap;
		try {
			HashMap<Integer, ArrayList<WBANStation>> fipsToWBANMap = climatology.FCCClient.getFIPSToWBANMap(wbanIter);
			msaToWBANMap = geography.MSAtoWBANMapper.getMSAToWBANMap(msaIter, msaToFIPSMap, fipsToWBANMap);
		} catch (IOException e) {
